...
class TestOcHelper(unittest.TestCase):
    ...
    def run_matrix_test(self, device):
        ...
        asso_indices = torch.tensor([3, 7,  3, -1, -1, 3,  7, 1, 1,  0, 0, -1,  1,  1,  -1,
                                    # 15 16  17  18  19 20  21...
                                     3, -1, 19, 3,  2, -1, 19], dtype=torch.int64, device=device)

        row_splits = torch.tensor(
            [0, 15, len(asso_indices)], dtype=torch.int64, device=device)
        ...
        self.assertTrue(torch.equal(obj_per_rs, torch.tensor(
            [4, 3], dtype=torch.int64, device=device)), "Test oc_helper_matrices obj per rs failed: data wrong")

        # check if device is correct
        self.assertTrue(M.device.type == device, "Test oc_helper_matrices M device failed: device wrong, got " +
                        M.device.type+" but expected "+device)
        self.assertTrue(M_not.device.type == device, "Test oc_helper_matrices M_not device failed: device wrong, got " +
                        M_not.device.type+" but expected "+device)

        # check if dtype is same as asso_indices
        self.assertTrue(M.dtype == asso_indices.dtype, "Test oc_helper_matrices M dtype failed: dtype wrong, got " +
                        str(M.dtype)+" but expected "+str(asso_indices.dtype))
        self.assertTrue(M_not.dtype == asso_indices.dtype, "Test oc_helper_matrices M_not dtype failed: dtype wrong, got " +
                        str(M_not.dtype)+" but expected "+str(asso_indices.dtype))

        # check if the shape is expected: M: 7 x 4, M_not: 7 x 14
        self.assertTrue(M.shape == (7, 4),
                        "Test oc_helper_matrices shape failed: shape wrong")
        self.assertTrue(M_not.shape == (7, 15),
                        "Test oc_helper_matrices shape failed: shape wrong")
        ...
        self.assertTrue(torch.equal(M_sorted, M_exp_sorted),
                        "Test oc_helper_matrices data failed: data wrong. Expected: \n"+str(M_exp_sorted)+"\nbut got\n"+str(M_sorted))

        self.assertTrue(torch.equal(M_not_sorted, M_not_exp_sorted),
    ...
    def run_select_with_default_test(self, device):
        ...
        # check if device is correct
        self.assertTrue(sel.device.type == device, "Test select_with_default self consistency failed: device wrong, got " +
                        sel.device.type+" but expected "+device)

        # check if dtype is same as asso_indices
        self.assertTrue(sel.dtype == asso_indices.dtype, "Test select_with_default self consistency failed: dtype wrong, got " +
                        str(sel.dtype)+" but expected "+str(asso_indices.dtype))

        # now every row in sel should contain the same value as M[:,0] or -100, test
        ok = sel == sel[:, 0:1]
        ok = ok | (sel == -100)
        self.assertTrue(torch.all(ok).item(
        ), "Test select_with_default self consistency failed: data wrong")
        ...
        self.assertTrue(sel.dtype == f_asso_indices.dtype, "Test select_with_default self consistency failed: dtype wrong, got " +
                        str(sel.dtype)+" but expected "+str(f_asso_indices.dtype))

        # now every row in sel should contain the same value as M[:,0] or -100, test
        ok = sel == sel[:, 0:1]
        ok = ok | (sel == -100.)  # now a float
        self.assertTrue(torch.all(ok).item(
        ), "Test select_with_default self consistency failed for float: data wrong")

        # now to a large scale test
        # create some random indices
        asso_indices = torch.randint(
            # -1 to have some -1s
            0, 1000, (100000,), dtype=torch.int64, device=device)-1
        # but take one row split only
        row_splits = torch.tensor(
            [0, len(asso_indices)], dtype=torch.int64, device=device)
        M, M_not, _ = oc_helper_matrices(asso_indices, row_splits)
        ...
        ok = ok | (sel == -100)
        self.assertTrue(torch.all(ok).item(
        ), "Test select_with_default large scale failed: data wrong")
        ...
        ok = ok | (sel == -100)
        self.assertTrue(torch.all(ok).item(
        ), "Test select_with_default large scale multi-dim failed: data wrong")
    ...
    def run_large_scale_test(self, device):
        ...
        asso_indices = torch.randint(
            0, 100, (1000,), dtype=torch.int64, device='cpu') - 1
        asso_indices = asso_indices.to(device)
        row_splits = torch.tensor([0, len(
            asso_indices)//4, len(asso_indices)//2, len(asso_indices)], dtype=torch.int64, device=device)
        M, M_not, _ = oc_helper_matrices(asso_indices, row_splits)
        ...
        self.assertTrue(torch.equal(M_sorted, M_exp_sorted),
                        f"Test oc_helper_matrices ({device}) large scale M data failed: data wrong.\n")
        self.assertTrue(torch.equal(M_not_sorted, M_not_exp_sorted),
                        f"Test oc_helper_matrices ({device}) large scale M_not data failed: data wrong.\n")

    @unittest.skipIf(not torch.cuda.is_available(), "CUDA not available")
    def test_cpu_against_cuda_matrices(self):
        def create_input():
            asso_indices = torch.randint(
                0, 100, (100000,), dtype=torch.int64, device='cpu') - 1
            row_splits = torch.tensor([0, len(
                asso_indices)//4, len(asso_indices)//2, len(asso_indices)], dtype=torch.int64, device='cpu')
            return asso_indices, row_splits

        asso_indices, row_splits = create_input()
        M_cpu, M_not_cpu, nper_cpu = oc_helper_matrices(
            asso_indices, row_splits)  # cpu
        # move inputs to cuda
        asso_indices = asso_indices.to('cuda')
        row_splits = row_splits.to('cuda')
        M_cuda, M_not_cuda, nper_cuda = oc_helper_matrices(
            asso_indices, row_splits)  # cuda
        ...
        M_cuda_sorted, M_not_cuda_sorted = self.sort_matrices(
            M_cuda, M_not_cuda)

        self.assertTrue(torch.equal(M_cpu_sorted, M_cuda_sorted),
                        "Test oc_helper_matrices cpu vs cuda failed: M data wrong")
        self.assertTrue(torch.equal(M_not_cpu_sorted, M_not_cuda_sorted),
                        "Test oc_helper_matrices cpu vs cuda failed: M_not data wrong")
        # needs sorting
