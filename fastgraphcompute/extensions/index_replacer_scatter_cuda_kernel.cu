...
torch::Tensor index_replacer_scatter_cuda_fn(
    ...
    TORCH_CHECK(sorting_indices.device() == idx_sorted.device(),
                "sorting_indices must be on the same device as idx_sorted");
    c10::cuda::CUDAGuard guard(idx_sorted.device());
    ...
    auto stream = c10::cuda::getCurrentCUDAStream(idx_sorted.device().index());
