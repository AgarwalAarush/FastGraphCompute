    ...
    int64_t mask_mode)
{
    ...
    CHECK_CUDA(mask);
    TORCH_CHECK(row_splits.device() == coords.device(),
                "row_splits must be on the same device as coords");
    TORCH_CHECK(mask.device() == coords.device(),
                "mask must be on the same device as coords");
    ...
    auto stream = c10::cuda::getCurrentCUDAStream(coords.device().index());

    AT_DISPATCH_FLOATING_TYPES(coords.scalar_type(), "set_defaults", ([&] {
        set_defaults <scalar_t> <<<grid, block, 0, stream.stream()>>> (
    ...
    C10_CUDA_CHECK(cudaMemcpyAsync(cpu_rowsplits.data(), row_splits.data_ptr<int64_t>(),
                                  n_rs * sizeof(int64_t), cudaMemcpyDeviceToHost,
                                  stream.stream()));
    ...
    for (int64_t j_rs = 0; j_rs < n_rs - 1; j_rs++) {
        ...
        AT_DISPATCH_FLOATING_TYPES(coords.scalar_type(), "select_knn_kernel", ([&] {
            select_knn_kernel <scalar_t> <<<n_blocks, block_size, 0, stream.stream()>>> (
